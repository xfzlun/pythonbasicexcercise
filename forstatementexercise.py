...
'''
#@Time  :2018-12-03;15:21
@author: aLuren

#@FileName: forstatementexercise.py
#@Software: PyCharm
...
(see Assignment statements), and then the suite is executed. When the items are exhausted
(which is immediately when the sequence is empty or an iterator raises a StopIteration exception),
the suite in the else clause, if present, is executed, and the loop terminates.

A break statement executed in the first suite terminates the loop without executing the else clause’s suite.
A continue statement executed in the first suite skips the rest of
the suite and continues with the next item, or with the else clause if there is no next item.
...
a = [11,22,33,44]
for i in range(len(a)):  #default初始值0已经省略，(0,len(a))
    print('第', i, '个元素为：', a[i])
...
'''
for循环不需要事先对循环变量i进行赋值，可以直接透过range对其进行初始的赋值
'''